...
def common_args(parser, monitoring=True):
    ...
    # Filtering args
    filtering_group.add_argument('-fkey', '--filter-keywords', type=str, help='Used to filter the search with specific keywords', nargs='+')
    ...
    filtering_group.add_argument('-flmsd', '--filter-last-modified-start-date', type=str, help='REQUIRES the "-flmed" argument to work. Used to filter the search with a start date for the "last modified date" key. Format: YYYY-MM-DDT00:00:00.000')
    filtering_group.add_argument('-flmed', '--filter-last-modified-end-date', type=str, help='REQUIRES the "-flmsd" argument to work. Used to filter the search with an end date for the "last modified date" key. Format: YYYY-MM-DDT00:00:00.000')
    filtering_group.add_argument('-fpsd', '--filter-published-start-date', type=str, help='REQUIRES the "-fped" argument to work. Used to filter the search with a start date for the published date. Format: YYYY-MM-DDT00:00:00.000')
    filtering_group.add_argument('-fped', '--filter-published-end-date', type=str, help='REQUIRES the "-fped" argument to work. Used to filter the search with an end date for the published date. Format: YYYY-MM-DDT00:00:00.000')

    # Options args
    if monitoring:
        options_group.add_argument('-lc', '--limit-cve', type=int, help='Limit the number of CVEs to be fetched', default=10)
        options_group.add_argument('-lr', '--limit-requests', type=int, help='Limit the number of requests to be made', default=5)
    ...
    # Debugging args
    debug_group.add_argument('-v', '--version', action='store_true', help='Show version information')
...
def parse_args():
    ...
    common_args(search_group, monitoring=False)
    ...
    common_args(continuous_group, monitoring=True)
